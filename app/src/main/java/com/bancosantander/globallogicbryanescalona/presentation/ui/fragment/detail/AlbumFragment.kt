...
class AlbumFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        with(viewModel) {
            observe(albumLiveData, ::albumObserver)
            observe(albumLocalLiveData, ::albumObserver)
        }
        ...
        if (connectionManager.isNetworkAvailable()) {
            ...
            viewModel.getLocalAlbum(collectionId)
        }
            ...
            is Result.OnSuccess -> {
                if (result.value.isNotEmpty()) {
                    for ((index, item) in result.value.withIndex()) {
                        if (result.value[index].collectionId == collectionId && result.value[index].wrapperType.equals(
                                "collection"
                            )
                        ) {
                            ...
                            songListAlbum.add(result.value[index])
                        }
                    }
